...
export async function PATCH(req: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    if (existing.status !== 'draft') {
      return NextResponse.json({ error: 'Only draft notices can be edited' }, { status: 409 })
    }
    ...
    const auditResult = await writeAdminAuditEvent({
      action:      'PATIENT_NOTICE_UPDATED',
      adminSub:    admin.sub,
      adminEmail:  admin.email,
      patientMsid: auditMsid,
      noteId:      noticeId,
      details:     `audience=${audience_type} placement=${placement} priority=${priority} previous_status=draft`,
    })
    if (!auditResult.ok) {
      return NextResponse.json({ error: 'Audit write failed — action aborted' }, { status: 500 })
    }

    const notice = await updatePatientNotice({
      noticeId,
      title,
      message,
    ...
    if (name === 'ConditionalCheckFailedException') {
      return NextResponse.json({ error: 'Notice could not be updated — it may have changed status' }, { status: 409 })
    }
    console.error('[notices] PATCH failed', err)
