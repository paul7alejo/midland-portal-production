...
import { expirePatientNotice, getPatientNotice, sanitizePatientNotice } from '@/lib/aws/patientNotices'
...
export async function POST(_req: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    if (existing.status !== 'published') {
      return NextResponse.json({ error: 'Only published notices can be expired' }, { status: 409 })
    }
    ...
    const auditResult = await writeAdminAuditEvent({
      action:      'PATIENT_NOTICE_EXPIRED',
      adminSub:    admin.sub,
      adminEmail:  admin.email,
      patientMsid: auditMsid,
      noteId:      noticeId,
      details:     `audience=${existing.audience_type} placement=${existing.placement} priority=${existing.priority} previous_status=published new_status=expired`,
    })
    if (!auditResult.ok) {
      return NextResponse.json({ error: 'Audit write failed — action aborted' }, { status: 500 })
    }

    const notice = await expirePatientNotice({ noticeId, adminSub: admin.sub, adminEmail: admin.email })
    ...
    if (name === 'ConditionalCheckFailedException') {
      return NextResponse.json({ error: 'Notice could not be expired — it may have changed status' }, { status: 409 })
    }
    console.error('[notices] expire failed', err)
    return NextResponse.json({ error: 'Failed to expire notice' }, { status: 500 })
  }
}
